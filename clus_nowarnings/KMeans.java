...
public class KMeans extends ClusteringAlgorithm
{
	...
	public euclidian_distance(Cluster cluster){
		for(int i = 0; i < trainData.size(); i++){
			double[] means = new double[cluster.currentMembers.size()];
			for(int s = 0; s < k; s++){
				double currentMean = 0;
				for(int j = 0; j < dim; j++){
					currentMean += sqrt(Math.pow(trainData.get(i)[j] - clusters[s].prototype[j], 2)); 
				}
				means[i] = currentMean;
			}
		}
	...
	public boolean train()
	{
		...
		///Step 1
		List<Integer> shuffeledData = new ArrayList<>(trainData);
		Collections.shuffle(shuffeledData);
		int clusterCount = -1;
		for(int i=0; i<shuffeledData.size(); i++) {
			if(i%(shuffeledData.size()/k)) {
				clusterCount++;
			}
			clusters[clusterCount].currentMembers.add(shuffeledData.get(i));
		}

		///Step 2
		for(int i = 0; i < k; i++){
			clusters[k].prototype = 
		}
